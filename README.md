...
void loop()
{
  ...
  // 안전 가드: a_value가 9 부근이면 비정상 큰 값 방지
  if (a_value <= 9.5f) {
  ...
  if (dist_raw >= _DIST_MIN && dist_raw <= _DIST_MAX) { // Put range Filter code here (_DIST_MIN ~ _DIST_MAX)
  ...
  dist_ema = EMA_ALPHA * dist_prev + (1.0f - EMA_ALPHA) * dist_ema; // Put EMA filter code here
  ...
  //duty = map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);
  {
    ...
    if (t < 0.0f) t = 0.0f;
    if (t > 1.0f) t = 1.0f;
    float duty_f = _DUTY_MIN + t * (_DUTY_MAX - _DUTY_MIN);
